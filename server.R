...
shinyServer(function(input, output) {
  ...
  output$summary <- renderPlotly({
      ...
      temp <- tally(group_by(testdata, hour(Date)))
  ...
  output$summary2 <- renderPlotly({
    ...
    temp <- tally(group_by(testdata, WeekDays))
    colnames(temp) <- c("Weekdays", "Count")
    temp$Weekdays <- factor(temp$Weekdays, levels = c("Sun", "Mon", "Tues", "Wed", "Thu", "Fri", "Sat"))
    temp <- arrange(temp, Weekdays)
    plot_ly(temp, x = Weekdays, y = Count)
  })
  
  output$trend <- renderPlotly({
    trend <- tally(group_by(Full_df, format(Date, "%Y %b")))
