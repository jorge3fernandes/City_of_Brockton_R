...
callParser <- function(txtparts){
  ...
  df <- data.frame(Time = timeOfDay, Date = logDate, timeStamp = timeStamp, operator = callTaker, 
                   dispAddress = dispAddress, officer = policeOff, callReasonAction = callReasonAction, 
...
CallInfoExtractor <- function(dayLog){
  ...
  do.call("rbind", apply(txtparts, 1, callParser))
...
pdfToTable <- function(pdfList){
  ...
  #
  textFile <- pdf_text(AllLinks[5]) # converting PDF to text
...
time1 <- proc.time()[3] - startTime

startTime <- proc.time()[3]
GetIndivDays_dplyr <- function(text){
