...
```{r , eval= FALSE, error= FALSE}
 df <- function(call_log) {
                    ...
                    BPD_log$Month <-   month(BPD_log$Date, label = TRUE)
...
curr_year$address <- as.character(curr_year$address)

curr_year$address[is.na(curr_year$address)] <- 0
...
for (ii in seq(startindex, length(curr_year$address))){
   print(paste("Working on index", ii, "of", length(curr_year$address)))
   #query the google geocoder - this will pause here if we are over the limit.
   result = getGeoDetails(curr_year$address[ii]) 
   print(result$status)     
   result$index <- ii
...
testdata <- curr_year[1:length(geocoded[,1]),]
