                            ...
                            BPD_log$Arr_summ_ind[i] <- 1
                              ...
                              if(!is.na(BPD_log$Summons[i])){
                                BPD_log$Arr_summ_ind[i] <- 1
                                  ...
                                  BPD_log$Arr_summ_ind[i] <- 0
                                  }
                      }
  
}
                  
                    BPD_log <- data.frame(lapply(BPD_log, as.character), stringsAsFactors=FALSE)
=======
...
str(testdata)
write.csv(toJSON(testdata),"test.csv")
...
mymap <- leaflet(data = testdata) %>% 
  ...
  addMarkers(~ long, ~ lat, popup = ~call_reason_action, markerClusterOptions(zoomToBoundsOnClick = TRUE)) 
mymap
...
BRCorpus <- Corpus(VectorSource(testdata$call_reason_action))
