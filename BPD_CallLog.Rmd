...
```{r , eval= FALSE, error= FALSE}
 df <- function(call_log) {
                    ...
                    BPD_log <- subset(BPD_log, !is.na(Date))
...
=======
infile <- "inputb"
>>>>>>> c2615a21c0849782c213d149cf5cf1721600138a
...
#Using the Data Science toolkit API since google gives a limit of 2500 addresses
library(httr)
library(rjson)

data <- paste0("[",paste(paste0("\"",Full_df$address,"\""),collapse=","),"]")
...
test <- testdata[,c(4,18)]

for ( i in seq_along(testdata$Summons)){
                      
                          if(!is.na(testdata$Summons[i])){
                            
                            testdata$Arr_summ_ind[i] <- "Summoned"
                          } else{
                              if(!is.na(testdata$Arrested[i])){
                                testdata$Arr_summ_ind[i] <- "Arrested"
                              } else {
                                  testdata$Arr_summ_ind[i] <- "other"
                                  }
                      }
...
mymap <- leaflet(data = testdata) %>% 
  ...
  addMarkers(~ long, ~ lat, popup = ~call_reason_action, clusterOptions = markerClusterOptions(zoomToBoundsOnClick = TRUE)) 
mymap
...
ts <- as.ts(testdata, order.by = testdata[,1]) %>% subset(!is.na(Date))

count(as.character(testdata$call_reason_action), vars = testdata$Date)
ts <- as.ts(ts, order.by =vars)

dygraph(ts, main = "Call Frequency", xlab = "Date", ylab = "Frequency")

unique(testdata$charges)
