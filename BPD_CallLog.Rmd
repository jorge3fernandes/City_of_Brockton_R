...
AllDays <- seq.Date(from = as.Date('2016-03-02'), to = Sys.Date(), by = "day")
...
filenames <- str_c(format(seq.Date(from = as.Date("2016-03-02"), 
...
mymap <- leaflet(data = check) %>% 
  addTiles() %>% 
  setView(-71.02016, 42.08667, zoom = 13) %>% 
  addMarkers(~ lon, ~ lat, clusterOptions = markerClusterOptions(zoomToBoundsOnClick = TRUE)) 
mymap
